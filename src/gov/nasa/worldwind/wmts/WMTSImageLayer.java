...
public class WMTSImageLayer extends BasicTiledImageLayer {
    private static final String[] formatOrderPreference = new String[]
            {
                    "image/dds", "image/png", "image/jpeg"
            };

    public WMTSImageLayer(AVList params) {
        super(params);
    }

    public WMTSImageLayer(Document dom, AVList params) {
        this(dom.getDocumentElement(), params);
    }

    public WMTSImageLayer(Element domElement, AVList params) {
        this(wmtsGetParamsFromDocument(domElement, params));
    }

    public WMTSImageLayer(WMTS100Capabilities caps, AVList params) {
        this(wmtsGetParamsFromCapsDoc(caps, params));
    }

    public WMTSImageLayer(String stateInXml) {
        this(wmtsRestorableStateToParams(stateInXml));
     ...
     */
    protected static AVList wmtsGetParamsFromDocument(Element domElement, AVList params) {
        if (domElement == null) {
     ...
     */
    public static AVList wmtsGetParamsFromCapsDoc(WMTS100Capabilities caps, AVList params) {
        if (caps == null) {
    ...
    public static class URLBuilder implements TileUrlBuilder {
        private static final String MAX_VERSION = "1.1.0";
        ...
        private final String wmtsVersion;
        ...
        public URLBuilder(AVList params) {
            ...
            this.styleName = params.getStringValue(AVKey.STYLE_NAMES);
            this.imageFormat = params.getStringValue(AVKey.IMAGE_FORMAT);
            this.tileMatrixIdentifiers = (List<String>) params.getValue("tileMatrixIdentifiers");
            this.URLTemplate = params.getStringValue("URL_Template");
            String version = params.getStringValue(AVKey.WMS_VERSION);

            String coordSystemKey;
            String defaultCS;
            if (version == null || WWUtil.compareVersion(version, "1.1.0") >= 0) {
                this.wmtsVersion = MAX_VERSION + "&";
                coordSystemKey = "&crs=";
                defaultCS = "CRS:84"; // would like to do EPSG:4326 but that's incompatible with our old WMS server, see WWJ-474
            } else {
                this.wmtsVersion = version;
                coordSystemKey = "&srs=";
                defaultCS = "EPSG:4326";
            }
        ...
        public URL getURL(Tile tile, String altImageFormat) throws MalformedURLException {
            ...
            if (level >= this.tileMatrixIdentifiers.size()) {
                System.err.println("urlForTile: null template, rowHeight, or tileMatrixIdentifiers");
                return null;
            }
            StringBuffer sb;
            if (this.URLTemplate == null) {
                ...
                if (!sb.toString().toLowerCase().contains("SERVICE=WMTS&"))
                    sb.append("&REQUEST=GetTile&");
                sb.append("&VERSION=").append(this.wmtsVersion);
                sb.append("&LAYER=").append(this.layerName).append("&");
                sb.append("&STYLE=").append(this.styleName).append("&");
                sb.append(this.crs);
...
//                sb.append("&styles=").append(this.styleNames != null ? this.styleNames : "");
                ...
                int flipRow = rowHeight - tile.getRow() - 1;

                String url = this.URLTemplate.replace(TILEMATRIX_TEMPLATE, this.tileMatrixIdentifiers.get(level));
                url = url.replace(TILEROW_TEMPLATE, flipRow + "");
                url = url.replace(TILECOL_TEMPLATE, tile.getColumn() + "");
                url = url.replace(STYLE_TEMPLATE, this.styleName);
                url = url.replace("{", "%7B");
                url = url.replace("}", "%7D");
    ...
    public void getRestorableStateForAVPair(String key, Object value,
        ...
        if (value instanceof URLBuilder) {
            rs.addStateValueAsString(context, "wms.Version", ((URLBuilder) value).wmtsVersion);
     ...
     */
    public static AVList wmtsRestorableStateToParams(String stateInXml) {
        if (stateInXml == null) {
