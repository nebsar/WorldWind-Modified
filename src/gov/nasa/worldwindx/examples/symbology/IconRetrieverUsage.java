...
/**
 * Example of using {@link IconRetriever} to retrieve the icon for a
 * MIL-STD-2525C symbol. This example retrieves several symbology icons as
 * BufferedImages and displays them in a JFrame. See the <a
 * href="https://worldwind.arc.nasa.gov/java/tutorials/icon-retriever/" target="_blank">Icon
 * Retriever Usage Guide</a> for more information on using IconRetriever.
 *
 * @author pabercrombie
...
public class IconRetrieverUsage {
    ...
    private static class AppFrame extends javax.swing.JFrame {
        ...
        protected IconRetriever modifierIconRetriever;
        ...
        public AppFrame() {
            ...
            this.modifierIconRetriever = new MilStd2525ModifierRetriever(iconRetrieverPath);
            ...
            WorldWind.getTaskService().addTask(new Runnable() {
                public void run() {
                    ...
                    BufferedImage image = iconRetriever.createIcon("SFGPEWRH--MTUSG", params);
                    SymbolCode symbolCode = new SymbolCode("SFGPEWRH--MTUSG");
                    try {
                        BufferedImage modifierImage = modifierIconRetriever.createIcon(symbolCode.getSymbolModifier(), params);
                        image.getGraphics().drawImage(modifierImage, 0, 0, null);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
